...
const BigCarousal = () => {
  const [array2, setArray2]: Array<any> = useState([
    {
  ...
  const [check, setCheck] : any = useState(0);

  useEffect(() => {
    check === 0 || check === 1 ? setTimeout(() => setNum(check), 500) : false;
  }, [check]);

  const changeNum = (e: number) => {
    setCheck(e);
  };
    ...
    <div className={`FirstCarousalDiv`}>
      <div className={`FirstCarousalMainDiv`}>
        ...
        <div
          className={
            num === 1
              ? "FirstCarousalItemDiv hidden"
              : check === 1
              ? "FirstCarousalItemDiv right"
              : num === 0
              ? `FirstCarousalItemDiv`
              : ""
          }
        >
          ...
          <div className={`FirstCarousalItemChildDiv1`}>
            ...
            <button className={`FirstCarousalItemButton`}>
              {array2[num].button}
            </button>
          </div>
        ...
        <div
          className={ num === 0 ? "FirstCarousalItemDiv hidden" : check === 0 ? "FirstCarousalItemDiv left" : 
          num === 1 ? `FirstCarousalItemDiv` : "" } >
          <div className={`FirstCarousalItemChildDiv1`}>
            <h1 className={`largeFont`}>{array2[num].heading}</h1>
            <p className={`bgGray3`}>{array2[num].para}</p>
            <button className={`FirstCarousalItemButton`}>
